...
public abstract class GunModel {
	...
	public void renderGun(MatrixStack matrix, ItemStack stack, IRenderTypeBuffer buffer, int light, int overlay) {
		matrix.pushPose();
	...
	public void renderStock(MatrixStack matrix, ItemStack stack, IRenderTypeBuffer buffer, int light, int overlay) {
		matrix.pushPose();
		translateRotateForPart(gun, matrix);
	...
	public void renderBody(MatrixStack matrix, ItemStack stack, IRenderTypeBuffer buffer, int light, int overlay) {
		matrix.pushPose();
		translateRotateForPart(gun, matrix);
	...
	public void renderBarrel(MatrixStack matrix, ItemStack stack, IRenderTypeBuffer buffer, int light, int overlay) {
		matrix.pushPose();
		translateRotateForPart(gun, matrix);
	...
	public void renderMag(MatrixStack matrix, ItemStack stack, IRenderTypeBuffer buffer, int light, int overlay) {
		Item magitem = gunitem.getMagPath(stack);
		
		IBakedModel model = ModelHandler.INSTANCE.getModel(magitem.getRegistryName());
		if (model != null) {
			matrix.pushPose();
	...
	public void renderHammer(MatrixStack matrix, ItemStack stack, IRenderTypeBuffer buffer, int light, int overlay) {
		if (getHammerPath() != null) {
			IBakedModel model = ModelHandler.INSTANCE.getModel(getHammerPath());
			if (model != null) {
				matrix.pushPose();
	...
	public void renderScope(MatrixStack matrix, ItemStack stack, IRenderTypeBuffer buffer, int light, int overlay) {
		IBakedModel model = ModelHandler.INSTANCE.getModel(ServerUtils.getScopePath(stack));
		if (model != null) {
				matrix.pushPose();
	...
	public void renderRightArm(MatrixStack matrix, Impl buffer, int light, AbstractClientPlayerEntity player) {
		matrix.pushPose();
	...
	public void renderLeftArm(MatrixStack matrix, Impl buffer, int light, AbstractClientPlayerEntity player) {
		matrix.pushPose();
		matrix.scale(0.7f, 1f, 1.6f);
	...
	public void do3rdPersonRendering() {

	}

	public void translateRotateForPart(GunModelPart part, MatrixStack matrix) {
	...
	public void translateHand(GunModelPart part, MatrixStack matrix) {
		matrix.translate(part.transform.getCombinedOffsetX(), part.transform.getCombinedOffsetY(),
				part.transform.getCombinedOffsetZ());
	}
	...
	public void debugAim(MatrixStack matrix, float p) {
		matrix.translate(MathHelper.lerp(p, 0, aim.transform.offsetX), MathHelper.lerp(p, 0, aim.transform.offsetY),
				MathHelper.lerp(p, 0, aim.transform.offsetZ));
		matrix.mulPose(new Quaternion(MathHelper.lerp(p, 0, aim.transform.rotationX),
				MathHelper.lerp(p, 0, aim.transform.rotationY), MathHelper.lerp(p, 0, aim.transform.rotationZ), false));
	}
	...
	public ItemMag getMagItem() {
		return (ItemMag) ClientEventHandler.getPlayer().inventory.getItem(ammoindex).getItem();
	}

	public ItemStack getMagStack() {
		return ClientEventHandler.getPlayer().inventory.getItem(ammoindex);
	}
	...
	public void rotateAroundXR(MatrixStack matrix, GunModelPart part, float angle) {
	...
	public void rotateAroundYR(MatrixStack matrix, GunModelPart part, float angle) {
	...
	public void rotateAroundZR(MatrixStack matrix, GunModelPart part, float angle) {
	...
	public void rotateAroundXD(MatrixStack matrix, GunModelPart part, float angle) {
		matrix.translate(0, part.transform.getCombinedOffsetY(), part.transform.getCombinedOffsetZ());
		matrix.mulPose(Vector3f.XP.rotationDegrees(angle));
		matrix.translate(0, -part.transform.getCombinedOffsetY(), -part.transform.getCombinedOffsetZ());
	}

	public void rotateAroundYD(MatrixStack matrix, GunModelPart part, float angle) {
		matrix.translate(part.transform.getCombinedOffsetX(), 0, part.transform.getCombinedOffsetZ());
		matrix.mulPose(Vector3f.YP.rotationDegrees(angle));
		matrix.translate(-part.transform.getCombinedOffsetX(), 0, -part.transform.getCombinedOffsetZ());
	}

	public void rotateAroundZD(MatrixStack matrix, GunModelPart part, float angle) {
		matrix.translate(part.transform.getCombinedOffsetX(), part.transform.getCombinedOffsetY(), 0);
	...
	protected void researchAmmoIndex(Item item) {
		ammoindex = ServerUtils.getIndexForItem(ClientEventHandler.getPlayer().inventory, item);
		magbullets = ServerUtils.getItemCount(ClientEventHandler.getPlayer().inventory, item);
	}
	...
	public abstract void handleAim(MatrixStack matrix, float aimProgress);

	public abstract void handleSprint(MatrixStack matrix, float aimProgress);
	...
	public abstract IBakedModel getModifiedModel(IBakedModel origin);
