...
public abstract class GunModel {
	...
	public void render(PoseStack matrix, ItemStack stack, MultiBufferSource itembuffer, BufferSource armbuffer, int light,
		...
		if (!(ServerUtils.hasScope(stack) && ClientEventHandler.handlers.get(ClientEventHandler.getPlayer().getUUID())
			...
			*/
			...
			float y = 0.0008f;
			float z = 0.0008f;
			matrix.translate(0, Mth.lerp(rec, 0, -x*0.25f)*(gunitem.getRVertRecoil(stack)*20), Mth.lerp(rec, 0, x*0.6f)*(gunitem.getZRecoil(stack)*20));//0.05f
	...
	public void setParts(GunModelPart[] parts) {
		this.parts = parts;
	}
	...
	public void initExtraParts(GunModelPart... parts) {
		extraParts = new String[parts.length];
		for(int i = 0; i < parts.length; i++) {
			extraParts[i] = parts[i].getName();
		}
	}
	...
	public void setModModel(WrapperModel model) {
		this.modModel = model;
	}
	...
	public abstract void initReloadAnimation(int currentgunbullets, int magbullets, boolean hasMag);
