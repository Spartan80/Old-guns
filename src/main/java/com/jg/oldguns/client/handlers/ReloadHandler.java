...
public class ReloadHandler {
	...
	public static void setMag(boolean mag, int type, String bpath) {
		OldGuns.channel.sendToServer(new MakeMagMessage(mag, type, bpath));
	}
	...
	public static void doGetOutMag(String mag, int bullets) {
		...
		setBullets(0);
		setMag(false, -1, Constants.EMPTY);
	}

	public static void doReloadMag(int magbullets, int ammoindex) {
		ItemStack magstack = (ClientEventHandler.getPlayer()).inventory.getItem(ammoindex);
		setBullets(magbullets);
		setMag(true, ((ItemMag) ClientEventHandler.getPlayer().inventory.getItem(ammoindex).getItem()).getMagType(),
				magstack.getOrCreateTag().getString(Constants.MAGBULLETPATH));
	...
	public static void doMag(int ammoindex) {
		ItemStack magstack = (ClientEventHandler.getPlayer()).inventory.getItem(ammoindex);
		setMag(true, ((ItemMag) ClientEventHandler.getPlayer().inventory.getItem(ammoindex).getItem()).getMagType(),
				magstack.getOrCreateTag().getString(Constants.MAGBULLETPATH));
	...
	public static void doGetOutMag(GunModel model) {
		...
		ReloadHandler.setMag(false, -1, "empty");
	}

	public static int findCorrectBullet(PlayerInventory pi, ItemStack stack) {
