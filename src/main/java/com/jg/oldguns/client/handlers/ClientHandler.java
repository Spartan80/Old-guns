...
public class ClientHandler {
	...
	public void render(PoseStack matrix, MultiBufferSource buffer, 
			MultiBufferSource.BufferSource impl, int light) {
		LocalPlayer player = Minecraft.getInstance().player;
		ItemStack stack = player.getMainHandItem();
		if(current != null) {
			current.tick(player, stack);
			current.render(player, stack, buffer, impl, matrix, light);
		}
	}
