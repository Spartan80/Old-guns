	...
	public void updateAimProgress(boolean aim) {
		...
		if (aim) {
			if (aimProgress < MAXPROGRESS) {
				...
				aimProgress = MathHelper.lerp(blend, aimProgress, 1);
				// aimProgress += (aimProgress - prevAimProgress + renderPartialTicks * 0.6f) ;
				// System.out.println(blend);
				/*
				 * aimProgress += (renderPartialTicks - (aimProgress - prevAimProgress * 1.2f));
				 * if(aimProgress > MAXPROGRESS) { aimProgress = MAXPROGRESS; }
				 */
				if (aimProgress > 0.9f) {
					aimProgress = 1;
				}
			...
			if (aimProgress > 0) {
				prevAimProgress = aimProgress;
				float blend = (float) (1f - Math.pow(1f - 0.1f, renderPartialTicks * 20f));
				aimProgress = MathHelper.lerp(blend, aimProgress, 0);
				// aimProgress -= (aimProgress - prevAimProgress + renderPartialTicks *
				// 0.6f);//MathHelper.smoothstep(1 * renderPartialTicks);
				/*
				 * aimProgress -= (renderPartialTicks - (aimProgress - prevAimProgress * 1.2f));
