	...
	public static void handle(ShootMessage msg, Supplier<Context> ctx) {
		...
		context.enqueueWork(() -> {
			...
			if(player != null) {
				...
				float f = -Mth.sin(msg.yaw * ((float)Math.PI / 180F)) * 
						Mth.cos(msg.pitch * ((float)Math.PI / 180F));
			      float f1 = -Mth.sin((msg.pitch) * ((float)Math.PI / 180F));
			      float f2 = Mth.cos(msg.yaw * ((float)Math.PI / 180F)) * 
			    		  Mth.cos(msg.pitch * ((float)Math.PI / 180F));
				...
				for(int i = 0; i < gun.getBulletsPerShoot(); i++) {
					...
					bullet.shoot((double)f, (double)f1, (double)f2, gun.getPower(), 
							gun.getInnacuracy());
				    Vec3 vec3 = player.getDeltaMovement();
				    bullet.setDeltaMovement(bullet.getDeltaMovement().add(vec3.x, 
				    		player.isOnGround() ? 0.0D : vec3.y, vec3.z));
