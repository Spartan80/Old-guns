...
public class MakeMagMessage {
	...
	public static void handle(MakeMagMessage msg, Supplier<Context> ctx) {
		...
		context.enqueueWork(() -> {
			...
			nbt.putBoolean(NBTUtils.HASMAG, msg.hasmag);
			if (msg.hasmag) {
				nbt.putInt(NBTUtils.MAXAMMO, msg.maxAmmo);
				nbt.putString(NBTUtils.MAGBULLET, msg.bpath);
				nbt.putString(NBTUtils.MAG, msg.mpath);
			}
		});
