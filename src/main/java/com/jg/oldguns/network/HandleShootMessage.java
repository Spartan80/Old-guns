		...
		context.enqueueWork(() -> {
			...
			if (player != null) {
				...
				if (!gun.isShotgun(stack)) {
					...
					System.out.println(bulletEntity.getDisplayName());*/
					Bullet bulletEntity = ((Bullet)EntityType.byString(
							stack.getOrCreateTag().getBoolean(Constants.HASMAG)
							? ((ItemMag) gun.getMagPath(stack)).getProjectile() : 
							gun.getProjectile(stack))
							.get().create(world));
					bulletEntity.init(player);
